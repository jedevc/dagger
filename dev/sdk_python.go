...
func (t PythonSDK) dev(opts ...dagger.PythonSDKDevOpts) *dagger.PythonSDKDev {
	src := bumpSDK("python", t.Version, t.Dagger.Source()).Directory(pythonSubdir)
	return dag.PythonSDKDev(src, opts...)
}

func (t PythonSDK) withVersion(version string) PythonSDK {
	t.Version = version
	return t
}
...
func (t PythonSDK) Publish(
	...
	result := t.withVersion(version).dev().Container().
...
func (t PythonSDK) Bump(
	ctx context.Context,
	// +optional
	version string,
) (*dagger.Directory, error) {
	result := bumpSDK("python", version, t.Dagger.Source())
